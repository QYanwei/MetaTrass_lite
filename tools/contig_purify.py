	...
	with open(quastAlnTsv, 'r') as file:
		for i in file:
			...
			if 'True' in i:
				...
				if name not in query_list.keys() and float(idy) >= float(IDY) and aligment_length >= 200:
					query_list[name] = aligment_length
				elif float(idy) >= float(IDY) and aligment_length >= 200:
					query_list[name] += aligment_length
				else:
					pass
	AF = float(PCT) / 100
...
if __name__ == '__main__':
	...
	parser.add_argument('-PCT', 			required=False,  type=str,      default = '50',	  help='Threshold of contig lnegth(0-100)')
