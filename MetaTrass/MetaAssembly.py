...
def supernova_assembly(tssfq1, tssfq2, memory, maprate, thread, maxreads, pairdepth, outdir, sample):
	...
	command2 = ' '.join([ config_dict['python'], config_dict['stlfr2supernova'], '-fastq1', tssfq1, '-fastq2', tssfq2, 
...
def cloudspades_assembly(tssfq1, tssfq2, memory, maprate, thread, pairdepth, outdir, sample):
	create_folder(outdir + '/dir3_assembly/')
	create_folder(outdir + '/dir3_assembly/cloudspades/')
	output = outdir + '/dir3_assembly/cloudspades/' + sample
	create_folder(output)

	command1 = ' '.join([ 'mkdir -p ', output, ' && cd ', output ])
	command2 = ' '.join([ config_dict['python'], config_dict['stlfr2cloudspades'], '-fastq1', tssfq1, '-fastq2', tssfq2,
		 '-memory', memory, '-maprate', maprate, '-thread', thread, '-pairdepth', pairdepth,
		 '-outdir', output, '-prefix', sample ])
	command3 = ' '.join([ 'rm -rf', output + '/cloudspades_out/' ])
	command4 = ' '.join([ 'rm -rf', output + '/longranger/' ])
	command5 = ' '.join([ 'rm -rf', output + '/*.fastq.gz'])
			...
			for tax in open(taxReadDepth, 'r').readlines():
				...
				if float(depth) >= float(max_depth):
					...
					if os.path.exists(tssfq1):
						task = cloudspades_assembly(tssfq1, tssfq2, memory, maprate, thread, pairdepth, outdir, taxid)
						TaskCMD.append(task)
						CMDFILE.write('%s\n' % ( task ) )
					else:
						pass
